...
- Version: **1.0.1**
- 发布日期: **2025/11/22**
- 概要: 将web完全前后端分离并增强健壮性
### 更新内容
- 修复 OTA 和各种页面中的错误处理, 现在可以在各种情况下返回对应的HTTP状态码了
- 实现 完全前后端分离的Web逻辑
- 优化 提升了提交 API key 后的校验严谨度, 现在会检测是否为符合格式的Base64 Ed25519 private key了
- 优化 独立的的PROGRAM网页存储
- 优化 通过响应式设计改进用户界面
- 添加 favicon (网站图标) 
- 添加 'site/server.js' 用于测试前端用的轻量化nodejs后端
- 修复 天气界面第二页和第三页有元素重复的问题
- 修复 从 URL 进行 OTA 时在 1602a 上显示字数超过16位的问题
- 优化 OTA 时的串口进度显示
